...
const { tripApi, itemApi, checkApi } = require('../../utils/api.js');
...
Page({
  ...
  // 删除查验记录（将checked设为0，并删除checked_at）
  ...
  onLoad(options) {
    ...
    const tripId = options ? options.tripId : null;
    if (!tripId) {
      wx.showToast({
        title: '缺少行程ID',
        icon: 'none'
  ...
  async fetchTripDetail() {
    ...
    try {
      ...
      if (response.success) {
        this.setData({ trip: response.data || {} });
      } else {
        wx.showToast({
          title: response.message || '获取行程信息失败',
          icon: 'none'
        });
      }
    } catch (error) {
  ...
  async fetchItems() {
    ...
    try {
      ...
      if (response.success) {
        ...
        const skippedItems = items.filter(item => item.checked === 2);
        const pendingItems = items.filter(item => item.checked === 0 || item.checked == null);
        const hasPending = pendingItems.length > 0;
        
        this.setData({
          allItems: items,
          checklist: items, // 用于显示所有物品
          checkedItems,
          skippedItems,
          pendingItems,
        ...
        // 更新进度和分类（updateProgress已包含分组逻辑）
        this.updateProgress();
      } else {
        wx.showToast({
          title: response.message || '获取物品失败',
          icon: 'none'
        });
      }
    } catch (error) {
      console.error('获取查验物品失败:', error);
      wx.showToast({
        title: '获取物品失败，请检查网络连接',
        icon: 'none'
      });
    } finally {
  ...
  async handleCheckItem() {
    const { currentItem, processingAction } = this.data;
    if (!currentItem || processingAction) return;

    this.setData({ processingAction: true });
    try {
      // checked: 1=已查验
      const response = await itemApi.updateCheckedStatus(currentItem.id, 1);
      if (response.success) {
        this.playSound('check');
        this.applyCheckResult(currentItem);
      } else {
      wx.showToast({
  ...
  applyCheckResult(item) {
    ...
    const updatedAll = this.data.allItems.map(each => {
      if (each.id === item.id) {
        return { ...each, checked: 1 };
      }
      return each;
    });

    const remainingPending = this.data.pendingItems.filter(pending => pending.id !== item.id);
    const updatedChecked = [...this.data.checkedItems, { ...item, checked: 1 }];

    const nextItem = remainingPending[0] || null;
    const nextStats = this.computeStats(
      updatedAll.length,
      updatedChecked.length,
      this.data.skippedItems.length
    );
    ...
    this.setData({
      allItems: updatedAll,
      checklist: updatedAll, // 同步更新checklist
      checkedItems: updatedChecked,
      pendingItems: remainingPending,
      currentItem: nextItem,
      stats: nextStats
    }, () => {
      // 更新进度和分类（updateProgress已包含分组逻辑），并恢复滚动位置
      this.updateProgress(currentScrollTop);
  ...
  async completeTrip() {
    const { tripId, completionRequesting, completionHandled } = this.data;
    if (!tripId || completionRequesting || completionHandled) {
      return;
    }
    try {
      this.setData({ completionRequesting: true });
      wx.showLoading({ title: '提交中...', mask: true });
      const resp = await checkApi.completeCheck(tripId);
      wx.hideLoading();
      if (resp.success) {
        this.playSound('complete');
        wx.showToast({ title: '封箱成功', icon: 'success', duration: 800 });
        this.setData({
          trip: {
            ...(this.data.trip || {}),
            status: 'completed'
      ...
      wx.hideLoading();
      console.error('完成查验失败:', error);
      wx.showToast({ title: '完成失败，请稍后再试', icon: 'none' });
      this.setData({ completionHandled: false, completionRequesting: false });
    }
  },
  
  // 更新进度（优化：统一更新所有数据，避免多次渲染导致滚动跳动）
  updateProgress(savedScrollTop) {
    ...
    uncheckedItems.forEach(item => {
      ...
      // 则通过关键词匹配来纠正分类（比如"香蕉"、"牛奶"、"葡萄"应该归到"食品类"）
  ...
  // 分类逻辑
  groupCategories() {
    const items = this.data.checklist || this.data.allItems;
    const categoryMap = {};
    
    // 分类映射（根据category字段或name推断）
    const categoryColors = {
      '证件': { color: 'blue', bg: 'blue' },
      '衣物': { color: 'green', bg: 'green' },
      '电子': { color: 'orange', bg: 'orange' },
      '洗漱': { color: 'purple', bg: 'purple' },
      '药品': { color: 'red', bg: 'red' },
      '其他': { color: 'pink', bg: 'pink' }
    };
    
    items.forEach(item => {
      // 尝试从category字段获取，如果没有则从name推断或使用"其他"
      ...
      if (!item.category) {
        ...
        if (name.includes('身份证') || name.includes('护照') || name.includes('签证') || name.includes('证件')) {
          ...
          category = '洗漱';
        } else if (name.includes('药') || name.includes('创可贴') || name.includes('感冒')) {
          category = '药品';
        }
      }
      ...
      if (!categoryMap[category]) {
        ...
        categoryMap[category] = {
          ...
          items: [],
          hasUnchecked: false,
          colorClass: `color-${colors.color}`,
          bgClass: `bg-${colors.bg}`
        };
      }
      
      categoryMap[category].items.push(item);
      if (!item.checked || item.checked === 0) {
        categoryMap[category].hasUnchecked = true;
      }
    });
  ...
  async handleCheckItemById(itemId, savedScrollTop) {
    ...
    if (!item || this.data.processingAction) return;
    
    this.setData({ processingAction: true });
    try {
      const response = await itemApi.updateCheckedStatus(itemId, 1);
      if (response.success) {
        ...
        this.data.checklist = updatedAll;
  ...
  async handleUncheckItem(item, savedScrollTop) {
    if (this.data.processingAction) return;
    
    this.setData({ processingAction: true });
    try {
      const response = await itemApi.updateCheckedStatus(item.id, 0);
      if (response.success) {
        // 直接更新数据，不单独 setData，让 updateProgress 统一处理
        const updatedAll = this.data.allItems.map(each => {
          if (each.id === item.id) {
            return { ...each, checked: 0 };
          }
          return each;
        });
  ...
  async handleSkipItem() {
    const { currentItem, pendingItems, skippedItems, processingAction } = this.data;
    if (!currentItem || processingAction) return;

    this.setData({ processingAction: true });
    try {
      // checked: 2=已跳过
      const response = await itemApi.updateCheckedStatus(currentItem.id, 2);
      if (response.success) {
        this.playSound('skip');
        
        const updatedAll = this.data.allItems.map(each => {
          if (each.id === currentItem.id) {
            return { ...each, checked: 2 };
          }
          return each;
        });

        const remainingPending = pendingItems.filter(pending => pending.id !== currentItem.id);
        const updatedSkipped = [...skippedItems, { ...currentItem, checked: 2 }];
        const nextStats = this.computeStats(
          updatedAll.length,
          this.data.checkedItems.length,
          updatedSkipped.length
        );
        const shouldAutoComplete = this.shouldPromptCompletion(nextStats);
      ...
      this.setData({
          ...
          currentItem: remainingPending[0] || null,
          stats: nextStats
        }, () => {
          if (shouldAutoComplete) {
            this.requestCompletionConfirmation();
  ...
  handleResumeSkipped() {
    ...
    const mergedQueue = [...pendingItems, ...skippedItems];
      this.setData({
      pendingItems: mergedQueue,
      skippedItems: [],
      currentItem: mergedQueue[0] || null,
